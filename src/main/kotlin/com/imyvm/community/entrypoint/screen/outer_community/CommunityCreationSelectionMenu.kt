...
class CommunityCreationSelectionMenu(
    ...
    init {
        ...
        if (isSelectionModeEnabled) {
            addButton(
                slot = 10,
                name = Translator.tr("ui.create.button.selection_mode.enable").string,
            ...
            if (isNormalSelectionMode) {
                addButton(
                    slot = 12,
                    name = (Translator.tr("ui.create.button.shape.prefix").string)
                            + when (currentShape) {
                                GeoShapeType.CIRCLE -> Translator.tr("community.shape.circle").string
                                GeoShapeType.POLYGON -> Translator.tr("community.shape.polygon").string
                                else -> Translator.tr("community.shape.rectangle").string
                            },
                ...
                addButton(
                    slot = 14,
                    name = Translator.tr("ui.create.button.exit_to_select").string,
            ...
            addButton(
                slot = 10,
                name = Translator.tr("ui.create.button.selection_mode.disable").string,
        ...
        if (hasEnoughPoints) {
            ...
            addButton(
                slot = 31,
                name = if (isCurrentCommunityTypeManor) Translator.tr("ui.create.button.type.manor").string
                else Translator.tr("ui.create.button.type.realm").string,
            ...
            addButton(
                slot = 34,
                name = Translator.tr("ui.create.button.confirm").string,
    ...
    companion object {
        private fun createMenuTitle(
            ...
            val pointCount = selectionState?.points?.size ?: 0
            val baseTitle = Translator.tr("ui.create.selection.title").string
            if (selectionState == null) {
                val hint = Translator.tr("ui.admin.region.global.add.hint.start").string
                return Component.literal("$baseTitle $hint")
            }
            if (!isNormalSelectionMode || pointCount < 2) {
                val hint = Translator.tr("ui.admin.region.global.add.hint.select").string
                return Component.literal("$baseTitle $hint")
            }
