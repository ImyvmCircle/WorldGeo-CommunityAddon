...
1. 本mod有i18n系统，通过`Translator.tr()`函数实现，以后中英文文件都要做。原则上不要使用`Text.literal()`。需实现`resource`里面对应的英文项目。对于发送给玩家的文本，需用MOTD格式制作比较美观的色彩和加粗下划线等效果，但不要引入Unicode特殊符号。对于`MANOR`和`RECTANGLE`等变量值，请用人类可读文本，转化成`manor`或`rectangle`等正常词汇文本引入消息，而不要直接放入消息。不要使用单引号包围参数，否则参数无法显示。
